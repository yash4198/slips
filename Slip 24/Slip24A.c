...
int main(int argc, char *argv[]) 
{
    int rank, size;
    int i;
    double local_odd_sum = 0.0, global_even_sum = 0.0;
    double numbers[1000];
    ...
    for (i = 0; i < 1000; i++) {
        ...
        {
            local_odd_sum += numbers[i];
        }
    }

    // Sum all local sums of even numbers to get the global sum
    MPI_Reduce(&local_odd_sum, &global_even_sum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);

    // Print result from root process
    if (rank == 0) 
    {
        printf("Sum of all odd randomly generated %d numbers: %lf\n", 1000 * size, global_even_sum);
    }
